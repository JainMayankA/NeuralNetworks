...
class CNN(object):
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         :return: None
         """
        self.model.add(layers.Dense(num_nodes,activation=activation,name=name,trainable=trainable))
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        self.model.add(layers.Conv2D(filters = num_of_filters,kernel_size = kernel_size, strides = strides,padding = padding, activation = activation,name = name,trainable=trainable))
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        self.model.add(layers.MaxPool2D(pool_size=pool_size,strides=strides,padding=padding,name=name))
        return self.model.layers[-1]
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        elif layer_names and type(str) == list:
    ...
    def remove_last_layer(self):
        ...
        """
        lastlayer_remove = self.model.pop()
        self.model = keras.Sequential(self.model.layers)
        return lastlayer_remove
